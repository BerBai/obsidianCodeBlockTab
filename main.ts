...
import { MarkdownPostProcessor, MarkdownPreviewRenderer } from "obsidian";
...
const DEFAULT_SETTINGS: CodeBlockTabsSettings = {
    extraLang: { "dataview": false, "dataviewjs": false, "java": true, "python": true, "c": true, "cpp": true, "go": true, "javascript": true },
    defaultTitle: "(x)"
}
...
export default class CodeBlockTabsPlugin extends Plugin {
    ...
    async onload() {
        ...
        let addTab = (el: HTMLElement) => {
            ...
            const lang: string | undefined = Array.from(codeElements[0].classList)
        ...
        this.registerMarkdownPostProcessor((el, ctx) => {
            ...
            if (el.childElementCount === 1) {
                ...
                switch (node.nodeName) {
                    case "PRE":
                        var codeElements = el.getElementsByTagName('code');
                        lang = Array.from(codeElements[0].classList)
                            .filter((a: string) => a.startsWith("language-"))
                            .map((l: string) => l.slice(9))[0]
                    ...
                    case "DIV":
                        lang = Array.from(node.classList)
